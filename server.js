...
io.on('connection', function (socket) {
  ...
  socket.on('token', function (object) {
    const { room, userId } = object
  ...
  socket.on('candidate', function (object, room) {
    logIt('Received candidate. Broadcasting...', room);
    // console.log(socket.id, 'socket.id')
    // console.log(candidate);
    socket.broadcast.to(room).emit('candidate', object);
  });
...
// Listen for Heroku port, otherwise just use 3388
var port = process.env.PORT || 3388;
